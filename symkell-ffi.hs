...
symkell_integrate varCs exprCs = safeFunc go (varCs, exprCs)
  where
    go (vCs, eCs) = do
      var <- cStringToText vCs
      exprMaybe <- parseExpression eCs
...
symkell_simplify exprCs = safeFunc go exprCs
  where
    go eCs = do
      exprMaybe <- parseExpression eCs
      case exprMaybe of
...
symkell_limit varCs targetCs exprCs = safeFunc go (varCs, targetCs, exprCs)
  where
    go (vCs, tCs, eCs) = do
      var <- cStringToText vCs
      exprMaybe <- parseExpression eCs
      targetMaybe <- parseExpression tCs
...
symkell_taylor_series exprCs varCs pointC orderC = safeFunc go (exprCs, varCs, pointC, orderC)
  where
    go (eCs, vCs, pC, oC) = do
      var <- cStringToText vCs
      exprMaybe <- parseExpression eCs
      let point = fromIntegral pC :: Integer
      let order = fromIntegral oC :: Int
      case exprMaybe of
...
symkell_laurent_series exprCs varCs pointC posC negC = safeFunc go (exprCs, varCs, pointC, posC, negC)
  where
    go (eCs, vCs, pC, posC, negC) = do
      var <- cStringToText vCs
      exprMaybe <- parseExpression eCs
      let point = fromIntegral pC :: Integer
